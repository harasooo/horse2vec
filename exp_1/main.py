...
class CustumBert(pl.LightningModule):
    def __init__(
        ...
        self.attn1 = nn.MultiheadAttention(self.d_model, num_heads=num_heads, batch_first=True)
        self.ln1 = nn.LayerNorm(self.d_model, self.layer_eps)
        self.attn2 = nn.MultiheadAttention(self.d_model, num_heads=num_heads, batch_first=True)
        self.ln2 = nn.LayerNorm(self.d_model, self.layer_eps)
        self.attn3 = nn.MultiheadAttention(self.d_model, num_heads=num_heads, batch_first=True)
        self.ln3 = nn.LayerNorm(self.d_model, self.layer_eps)
        self.attn4 = nn.MultiheadAttention(self.d_model, num_heads=num_heads, batch_first=True)
    ...
    def forward(self, horses, covs, pad_mask):
        ...
        hidden_states = self.attn1(atten_inputs, atten_inputs, atten_inputs, key_padding_mask=pad_mask)[0]
        hidden_states = self.ln1(hidden_states)
        hidden_states = self.attn2(atten_inputs, atten_inputs, atten_inputs, key_padding_mask=pad_mask)[0]
        hidden_states = self.ln2(hidden_states)
        hidden_states = self.attn3(atten_inputs, atten_inputs, atten_inputs, key_padding_mask=pad_mask)[0]
